<?php
    ...
    $q="SELECT * FROM (SELECT        TOP (100) PERCENT commessa_numero_cabina, codice_cabina AS codice_componente, fase, SUM(qnt) AS qnt, val
...
UNION
SELECT        TOP (100) PERCENT CASE WHEN codcabina_gestionale LIKE '+V%' THEN codcabina_gestionale WHEN codice_corridoio_ponte_zona LIKE '+V%' THEN codice_corridoio_ponte_zona END AS commessa_numero_cabina, 
                         cabine.codice_kit, 'S020' AS fase, ISNULL(cabine.qnt, 0) - ISNULL(sottoinsiemi_corridoi.qnt, 0) AS qnt, 'qq' AS val
FROM            mi_webapp.dbo.consistenza_commesse INNER JOIN
                             (SELECT        cabine_1.codice_cabina, kit_1.codice_kit, mi_db_tecnico.dbo.kit_cabine.qnt
                               FROM            mi_db_tecnico.dbo.cabine AS cabine_1 INNER JOIN
                                                         ...
                                                         mi_db_tecnico.dbo.kit AS kit_1 ON mi_db_tecnico.dbo.kit_cabine.id_kit = kit_1.id_kit) AS cabine ON mi_webapp.dbo.consistenza_commesse.nr_codice_pareti_kit = cabine.codice_cabina LEFT OUTER JOIN
                             (SELECT        cabine_2.codice_cabina, mi_db_tecnico.dbo.kit.codice_kit, SUM(mi_db_tecnico.dbo.kit_sottoinsiemi_corridoi.qnt) AS qnt
                               FROM            mi_db_tecnico.dbo.kit INNER JOIN
                                                         ...
                                                         mi_db_tecnico.dbo.sottoinsiemi_corridoi AS sottoinsiemi_corridoi_1 ON mi_db_tecnico.dbo.kit_sottoinsiemi_corridoi.id_sottoinsieme_corridoio = sottoinsiemi_corridoi_1.id_sottoinsieme_corridoio INNER JOIN
                                                         mi_db_tecnico.dbo.cabine AS cabine_2 ON sottoinsiemi_corridoi_1.id_cabina = cabine_2.id_cabina
                               GROUP BY mi_db_tecnico.dbo.kit.codice_kit, cabine_2.codice_cabina) AS sottoinsiemi_corridoi ON cabine.codice_kit = sottoinsiemi_corridoi.codice_kit AND cabine.codice_cabina = sottoinsiemi_corridoi.codice_cabina
WHERE        (ISNULL(cabine.qnt, 0) - ISNULL(sottoinsiemi_corridoi.qnt, 0) > 0) AND (mi_webapp.dbo.consistenza_commesse.commessa = $id_commessa) AND (mi_webapp.dbo.consistenza_commesse.origine = 'tip_cor')) AS t
